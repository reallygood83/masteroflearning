...
const grokClient = new OpenAI({
  apiKey: process.env.XAI_API_KEY || 'dummy-key', // fallback to prevent initialization error
...
async function generateWithGrok(
  ...
  const response = await grokClient.chat.completions.create({
    model: 'grok-4-1-fast', // 빠른 응답을 위한 Grok 4.1 Fast 모델
    messages: [
      {
        role: 'system',
...
async function generateWithGPT4(
  ...
  if (!openaiClient) {
    throw new Error('OpenAI API key is not configured. Cannot use GPT-4 fallback.');
  }
...
언어: ${rawNews.country === 'US' ? '영어 (한글로 번역 필요)' : '한국어'}

**중요: 영어 기사인 경우 반드시 한글로 번역하여 작성하세요!**

**파인만 기법 4단계:**

1단계: 선택 (무엇을 설명할 것인가)
- 이 뉴스의 핵심 개념을 한 문장으로 정의하세요
- 영어 기사라면 먼저 내용을 이해하고 한글로 설명하세요
...
**출력 형식 (반드시 JSON, 모든 텍스트는 한글):**

\`\`\`json
{
  "feynmanTitle": "궁금증을 유발하는 질문 형식 제목 (30자 이내, 한글)",
  "feynmanSummary": "초등학생도 이해 가능한 한 줄 요약 (비유 활용, 한글)",
  "feynmanContent": "마크다운 형식 본문 (## 헤더 사용, 800-1200자, 한글)",
  "questions": [
    {
